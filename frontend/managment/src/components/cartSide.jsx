...
const CartSidebar = () => {
  ...
  setMpesaPaymentStatus('awaiting_confirmation');
    ...
    /*const response = await fetch("http://localhost:5000/api/mpesa/stk-push", {
      ...
      body: JSON.stringify({
        phone: paymentData.mpesaPhoneNumber.replace(/^0/, "254"), // convert to international format
        amount: cartTotal || 1 // you can pull this from state
      })
    });
   ...
   const response = await fetch("http://localhost:5000/api/mpesa/stk-push", {
    ...
    body: JSON.stringify({
        phone: "254708374149", // or paymentData.mpesaPhoneNumber
        amount: cartTotal || 100
    })
    });

    console.log("📤 Sending request to backend...", {
    phone: paymentData.mpesaPhoneNumber.replace(/^0/, "254"),
    amount: cartTotal || 1,
    });   

    const data = await response.json();
    if (data.ResponseCode === "0") {
      alert("✅ STK Push Sent to Test Number (254708374149). Simulate approval.");
    ...
    alert("Payment request failed. Check server.");
    setMpesaPaymentStatus('failed');
  } 
  finally {
    setProcessing(false);
 ...
 // fallback to 1 if total is missing
    ...
    if (data.ResponseCode === "0") {
      alert("✅ STK Push Sent. Simulate approval in Daraja.");
      setMpesaPaymentStatus('confirmed');
    } else {
      alert(`❌ Payment failed: ${data.errorMessage || 'Unknown error'}`);
      setMpesaPaymentStatus('failed');
...
const initiateMpesaPaymentSimulation = async () => {
  ...
  try {
   ...
   /* const initiateMpesaPaymentSimulation = async () => {
    if (
        ...
        !/^07\d{8}$/.test(paymentData.mpesaPhoneNumber)
    ) {
        alert("Please enter a valid 10-digit M-Pesa phone number (e.g., 07XXXXXXXX).");
        return;
    }
    ...
    try {
        ...
        body: JSON.stringify({
            phone: paymentData.mpesaPhoneNumber.replace(/^0/, "254"), // e.g. 0743... → 254743...
            amount: cartTotal || 2000, // Or get from your cart state
        }),
        });
        ...
        if (data.ResponseCode === "0") {
        alert("✅ STK Push Sent to Test Number. Simulate payment on Daraja.");
        setMpesaPaymentStatus("confirmed");
        } else {
        alert(`❌ Payment failed: ${data.errorMessage || "Unknown error"}`);
        setMpesaPaymentStatus("failed");
  ...
  /*  const initiateMpesaPaymentSimulation = async () => {
    ...
    try {
        ...
        body: JSON.stringify({
            phone: paymentData.mpesaPhoneNumber.replace(/^0/, "254"),
            amount: 2000  // Replace with cartTotal later
        })
        });
